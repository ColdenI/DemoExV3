...
{
    public partial class RealtorMainForm : Form
    {
        ...
        private void RealtorMainForm_Load(object sender, EventArgs e)
        {
            DrawTable();
        }
                ...
                using (var query = conn.CreateCommand())
                {
                    ...
                    using (var reader = query.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            Console.WriteLine(Shift.GetShiftNow().datestart.ToString());
                            Console.WriteLine(Shift.GetShiftNow().dateend.ToString());
                            Console.WriteLine(order.datecreation.ToString());
                            Console.WriteLine();
                            Console.WriteLine();

                            if(!(Shift.GetShiftNow().datestart <= order.datecreation && Shift.GetShiftNow().dateend >= order.datecreation)) continue;
        ...
        private void toolStripButton3_Click(object sender, EventArgs e)
        {
            using (var conn = new MySqlConnection(Program.SQLBuilder.ConnectionString))
            {
                ...
                {
                    query.CommandTimeout = 30;
                    query.CommandText = "UPDATE `order` SET `paymentstatus` = 'принят' WHERE `orderid` = @id AND `paymentstatus` <> 'оплачен';";
        ...
        private void toolStripButton4_Click(object sender, EventArgs e)
        {
            using (var conn = new MySqlConnection(Program.SQLBuilder.ConnectionString))
            {
                ...
                {
                    query.CommandTimeout = 30;
                    query.CommandText = "UPDATE `order` SET `paymentstatus` = 'оплачен' WHERE `orderid` = @id;";
                    query.Parameters.AddWithValue("@id", (int)dgv.CurrentCell.OwningRow.Cells[0].Value); // получение id из таблицы
                    query.ExecuteNonQuery();
                }
            }
